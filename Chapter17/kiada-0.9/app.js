...
const nodeAgentURL = process.env.NODE_AGENT_URL;
    ...
    switch (req.url) {
        ...
        case '/text':
            Promise.allSettled([
                ...
                let nodeInfo = results[2].status === "fulfilled"
                    ? results[2].value
        ...
        case '/html':
            Promise.allSettled([
                ...
                let nodeInfo = results[0].status === "fulfilled"
                    ? results[0].value
                    : ("ERROR: " + results[0].reason)
                let map = {
                    "{{nodeInfo}}": nodeInfo,
                };
                return renderFile(req, res, "html/index.html", "text/html", map);
            });
            break;
        case '/stylesheet.css':
...
log("URL of the node-agent service is " + nodeAgentURL);
